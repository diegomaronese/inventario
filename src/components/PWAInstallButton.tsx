...
export const PWAInstallButton: React.FC<PWAInstallButtonProps> = ({
  ...
  const handleClick = async () => {
    if (isInstallable) {
      ...
      if (!outcome) {
        // User dismissed or prompt didn't trigger, show guide as fallback
        ...
        >
          ...
          >
            ...
            {guideType === 'ios' ? (
              ...
              <div className="space-y-3 text-xs text-zinc-700 dark:text-zinc-300">
                {isInAppBrowser && (
                  <div className="p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-900 dark:text-amber-200 text-[11px] leading-relaxed">
                    <strong>Navegador interno detectado:</strong> Você está visualizando pelo WhatsApp/Instagram. Para instalar como app, toque no menu <strong>⋮</strong> e escolha <strong>"Abrir no Chrome"</strong>.
                  </div>
                )}
                ...
                <div className="flex items-start gap-2.5 p-2.5 rounded-xl bg-zinc-50 dark:bg-zinc-950 border border-zinc-200/80 dark:border-zinc-800">
                  ...
                  <p className="flex-1 leading-relaxed">
                    No <strong>Google Chrome</strong> (ou Edge / Samsung Internet), toque no menu de <strong>três pontos (⋮)</strong> no canto superior direito.
                  </p>
                </div>
                ...
                <div className="flex items-start gap-2.5 p-2.5 rounded-xl bg-zinc-50 dark:bg-zinc-950 border border-zinc-200/80 dark:border-zinc-800">
                  ...
                  <div className="flex-1 leading-relaxed">
                    <p>
                      Selecione a opção <strong className="text-amber-600 dark:text-amber-400">"Instalar aplicativo"</strong> (ou <em>"Instalar Inventário"</em>).
                    </p>
                    <p className="mt-1 text-[11px] text-zinc-500 dark:text-zinc-400">
                      Evite a opção apenas "Criar atalho", pois ela mantém a barra de navegação do navegador. A opção <strong>"Instalar aplicativo"</strong> gera o app nativo real (WebAPK).
                    </p>
                  </div>
                </div>

                <div className="flex items-start gap-2.5 p-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-950 dark:text-emerald-200">
                  <CheckCircle2 className="w-4 h-4 text-emerald-500 flex-shrink-0 mt-0.5" />
                  <p className="flex-1 leading-relaxed text-[11px]">
                    Após a instalação, abra o app pelo ícone oficial na sua tela de aplicativos. Ele abrirá em <strong>tela cheia</strong>, sem barra de navegação, idêntico a um app nativo.
                  </p>
                </div>
              </div>
            )}
